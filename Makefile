...
upProduction: up
	...
	mv ./y2bsearch/.env.prod ./y2bsearch/.env
down:
	docker-compose down
stop:
	docker-compose stop
...
crawlVideos:
	docker exec -it y2search_crawler bash -c "go run app/videosCrawler.go"
crawlVideosBG:
	docker exec -it -d y2search_crawler bash -c "go run app/videosCrawler.go"
